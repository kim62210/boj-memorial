...
async function writeHealthResponse(res: ServerResponse): Promise<void> {
  try {
    ...
    console.error('/health DB ping failed:', msg)
    res.writeHead(503, {
      'Content-Type': 'text/plain; charset=utf-8',
      'Cache-Control': 'no-store',
    })
...
async function main(): Promise<void> {
  ...
  const httpServer = createServer((req: IncomingMessage, res: ServerResponse) => {
    ...
    // /health 는 Caddy blue/green probe 계약과 동일하게 text/plain 으로 직접 응답한다.
    if (pathname === '/health' || pathname === '/healthz') {
      void writeHealthResponse(res)
      return
    }
